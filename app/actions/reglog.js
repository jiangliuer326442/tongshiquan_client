...
export const GETCOMPANY = 'setcarduser';
...
export function setcompany(selectedcompany, dispatch){
	dispatch({
	  type: SETCOMPANY,
...
export function sendregcode(uid,token,phone,password,dispatch,cb){
	  dispatch({
		type: SENDREGCODE,
		phone: phone,
		password: password,
...
//手机哈注册
export function send_register(companyid,phone,codes,password,cb){
  common.ajax({
	url: server_url + "/registerbyphone.jsp?",
	type:"POST",
	data:{companyid: companyid, phone: phone, codes: codes, password: password, vendor: "", uuid: ""},
	success:function(status, info, data){
		cb(status,info,data);
	}
  });
